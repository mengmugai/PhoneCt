...
import com.mmg.phonect.settings.adapters.AppListTrendDisplayAdapter;
...
public class AppListTrendDisplayManageActivity extends GeoActivity {

    private ActivityDailyTrendDisplayManageBinding mBinding;

    private AppListTrendDisplayAdapter mAppListTrendDisplayAdapter;
    private ItemTouchHelper mAppListTrendDisplayItemTouchHelper;
    ...
    private class AppListTrendTag implements TagAdapter.Tag {

        AppListTrendDisplay tag;

        AppListTrendTag(AppListTrendDisplay tag) {
            this.tag = tag;
        }

        @Override
        public String getName() {
            return tag.getName(AppListTrendDisplayManageActivity.this);
        }
    }
    ...
    private class CardDisplaySwipeCallback extends SlidingItemTouchCallback {
        ...
        public boolean onMove(@NonNull RecyclerView recyclerView,
            ...
            mAppListTrendDisplayAdapter.moveItem(fromPosition, toPosition);
            return true;
        }

        @Override
        public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
            mAppListTrendDisplayAdapter.removeItem(viewHolder.getAdapterPosition());
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<AppListTrendDisplay> displayTags
                = SettingsManager.getInstance(this).getAppListTrendDisplay();
        mAppListTrendDisplayAdapter = new AppListTrendDisplayAdapter(

                displayTags,
                appListTrendDisplay -> {
                    setResult(RESULT_OK);
                    mTagAdapter.insertItem(new AppListTrendTag(appListTrendDisplay));
                    resetBottomBarVisibility();
                },
                holder -> mAppListTrendDisplayItemTouchHelper.startDrag(holder)
        );
        ...
        mBinding.recyclerView.setAdapter(mAppListTrendDisplayAdapter);

        mAppListTrendDisplayItemTouchHelper = new ItemTouchHelper(new CardDisplaySwipeCallback());
        mAppListTrendDisplayItemTouchHelper.attachToRecyclerView(mBinding.recyclerView);

        List<AppListTrendDisplay> otherTags = new ArrayList<>();
        otherTags.add(AppListTrendDisplay.TAG_XPOSEDMODULE);
        otherTags.add(AppListTrendDisplay.TAG_HOOK_FRAMEWORK);
        ...
        for (AppListTrendDisplay tag : otherTags) {
            tagList.add(new AppListTrendTag(tag));
        }
        ...
        mTagAdapter = new TagAdapter(
                ...
                (checked, oldPosition, newPosition) -> {
                    setResult(RESULT_OK);
                    AppListTrendTag tag = (AppListTrendTag) mTagAdapter.removeItem(newPosition);
                    mAppListTrendDisplayAdapter.insertItem(tag.tag);
                    resetBottomBarVisibility();
                    return true;
                }
        );
    ...
    protected void onStop() {
        ...
        List<AppListTrendDisplay> oldList = SettingsManager.getInstance(this).getAppListTrendDisplay();
        List<AppListTrendDisplay> newList = mAppListTrendDisplayAdapter.getDailyTrendDisplayList();
        if (!oldList.equals(newList)) {
            SettingsManager.getInstance(this).setAppListTrendDisplay(newList);
        }
    }
