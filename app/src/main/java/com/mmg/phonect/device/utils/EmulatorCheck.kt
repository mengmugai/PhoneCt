...
class EmulatorCheck(protected val context: Context){
    val name = "Emulator Check"
    ...
    fun checkEmulator(): String {


        if (buildChecks()) return "Build存在虚拟机痕迹"
        else if (checkSusFiles()) return "存在虚拟机相关文件"
        else  if (checkCPUInfo()) return "CPU含有虚拟机痕迹"
        else if (checkNetworkOperatorName()) return "网络运营商名称异常"
