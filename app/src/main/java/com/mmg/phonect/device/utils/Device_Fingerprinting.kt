...
//https://github.com/thouger/android_study/blob/master/app/src/main/java/thouger/study/Device_Fingerprinting.kt
...
class DeviceInfoUtils {
    ...
    fun printALL(context: Context,flag: String): MutableMap<String, String> {
        ...
        Log.d("tag",appsfly_map.toString())
...
//                        Log.d("tag","sensor value changed: ${event.values[0]}")
    ...
    fun getSerialNumber(): String {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            Log.d("tag","Android 11及更高版本，无法直接获取SerialNumber号")
            "null"
    ...
    fun getSIm(context: Context): String {
        ...
        Log.d("tag","activeSimCount:"+activeSimCount)
        ...
        if (activeSubList.isNotEmpty()) {
            for (subscriptionInfo in activeSubList) {
                ...
                Log.d("tag","simSlotIndex: $simSlotIndex, displayName: $displayName, carrierName: $carrierName, iccId: $iccId, countryIso: $countryIso"
                )
            }
        }
        try {
        ...
        for (field in allStaticFields) {
            if (field.toString().contains("sNameValueCache")) {
                ...
                for (method in HiddenApiBypass.getDeclaredMethods(nameValueCacheObj::class.java)) {
                    Log.d("tag","NameValueTable method: $method")
                    if (method.toString().contains("putStringForUser")) {

                        Log.d("tag","ApplicationInfo method: $method")
                        ...
                        if (_method.name == "putStringForUser") {
                            ...
                            Log.d("tag","result1111111111111: " + androidid)
                        }
                    }
                }
    ...
    fun getIMEI(context: Context): String {
        ...
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            // Android 10及更高版本，无法直接获取IMEI号，给出提示
            Log.d("tag","Android 10及更高版本，无法直接获取IMEI号")
            "null"
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            tm.imei
        } else {
